...
class CoteTopologyState:
    ...
    @classmethod
    def dense_initial(cls, normalize_inbound: bool = False) -> "CoteTopologyState":
        weights: Dict[Edge, float] = {}
        for src in NODE_KEYS:
            for dst in NODE_KEYS:
                weights[(src, dst)] = 0.0 if src == dst else 1.0 / NODE_COUNT
        topology = cls(weights=weights)
        if normalize_inbound:
            topology.normalize_inbound()
        return topology
    ...
    def from_env(cls) -> "CoteTopologyState":
        ...
        normalize = os.environ.get("COTE_STRICT_REPRO", "0").strip().lower() not in {
            "0",
            "false",
            "no",
            "off",
            "",
        } or os.environ.get("COTE_NORMALIZE_TOPOLOGY", "0").strip().lower() not in {
            "0",
            "false",
            "no",
            "off",
        ...
        return cls.dense_initial(normalize_inbound=normalize)
