...
from .gradient_update import apply_bidirectional_gradient_refinement, project_inbound_simplex
from .joint_fitness import StrictCoteFitnessEvaluator
from .local_model import LocalCausalLM
from .message_semantics import parse_message_distribution
from .model_pool import StrictModelPool
...
from .strict_el_bdpea import StrictELBDPEA
...
class PaperCOTEAgent:
    ...
    def __init__(self, client_id: Optional[int] = None, seed: Optional[int] = None) -> None:
        ...
        if self.hp.strict_repro:
            self.topology.normalize_inbound()
        ...
        self.evaluator = (
            StrictCoteFitnessEvaluator(self.hp, self.rng)
            if self.hp.strict_repro
            else CoteFitnessEvaluator(self.hp, self.rng)
        )
        self.evolver = StrictELBDPEA(self.hp, self.rng) if self.hp.strict_repro else ELBDPEA(self.hp, self.rng)
        self.local_model = LocalCausalLM()
        self.strict_model_pool = StrictModelPool.from_env(self.local_model) if self.hp.strict_repro else None
        ...
        self.node_model_used = 0
        self.node_model_successes = 0
        self.node_model_failures = 0
        self.node_model_by_node: Dict[str, Dict[str, int]] = {
            node: {"attempts": 0, "successes": 0, "failures": 0}
    ...
    def select_action(self, msg: Mapping[str, Any]) -> int:
        ...
        node_thoughts = self._run_local_node_thinking(context, candidates[: min(8, len(candidates))], edge_records)
        if node_thoughts:
            candidates = self._apply_local_node_scores(candidates, node_thoughts)
            action_distribution = self._candidate_distribution(candidates, len(actions))
        ...
        for edge in self.topology.active_edges():
            ...
            records.append(
                edge_record_from_message(
                    ...
                    after,
                    weight,
                    strict_parse=self.hp.strict_repro,
                )
            )
        return records

    def _run_local_node_thinking(
        self,
        context: GameContext,
        candidates: Sequence[Candidate],
        edge_records: Sequence[EdgeRecord],
    ) -> Dict[str, Any]:
        if not self._should_ask_local_nodes(candidates):
            return {}
        history_tail = self.belief.history_tail() if self.hp.belief_channel else []
        edge_messages = [
            {
                "edge": f"{record.edge[0]}->{record.edge[1]}",
        ...
        for node_key in NODE_KEYS:
            if node_key == OUTPUT_NODE:
                continue
            if not self._node_budget_available():
                break
            self.node_model_used += 1
            stats = self.node_model_by_node.setdefault(node_key, {"attempts": 0, "successes": 0, "failures": 0})
            stats["attempts"] += 1
            thought = self.local_model.think_node(
                node_key,
                context,
                candidates,
                history_tail,
            ...
            if thought is None:
                self.node_model_failures += 1
                stats["failures"] += 1
                continue
            thoughts[node_key] = thought
            self.node_model_successes += 1
            stats["successes"] += 1
        return thoughts

    def _should_ask_local_nodes(self, candidates: Sequence[Candidate]) -> bool:
        if not self.hp.node_local_model or not self.hp.use_local_model or not self.local_model.enabled:
            return False
        if not candidates:
            return False
        return self._node_budget_available()

    def _node_budget_available(self) -> bool:
        budget = int(self.hp.node_local_model_budget)
        return budget <= 0 or self.node_model_used < budget

    def _apply_local_node_scores(
        self,
        candidates: Sequence[Candidate],
        node_thoughts: Mapping[str, Any],
    ) -> List[Candidate]:
        adjusted: List[Candidate] = []
        for candidate in candidates:
            bonus = 0.0
            node_scores = dict(candidate.node_scores)
            for node_key, thought in node_thoughts.items():
                local_score = self._local_candidate_score(thought, candidate.index)
                if local_score is None:
                    continue
                confidence = self._local_confidence(thought)
                contribution = float(self.hp.node_local_score_scale) * confidence * local_score
                bonus += contribution
                node_scores[node_key] = node_scores.get(node_key, 0.0) + contribution
            adjusted.append(
                Candidate(
                    candidate.index,
                    candidate.action,
                    candidate.base_score + bonus,
                    node_scores,
                    candidate.probability,
                    candidate.reason,
                )
            )
        adjusted.sort(key=lambda item: (item.base_score, self.rng.random() * 0.001), reverse=True)
        return adjusted

    @staticmethod
    def _local_candidate_score(thought: Any, candidate_index: int) -> Optional[float]:
        if not isinstance(thought, Mapping):
            return None
        raw = thought.get("candidateScores")
        value: Any = None
        if isinstance(raw, Mapping):
            value = raw.get(str(candidate_index), raw.get(candidate_index))
        elif isinstance(raw, list) and 0 <= candidate_index < len(raw):
            value = raw[candidate_index]
        try:
            if value is None:
                return None
    ...
    @staticmethod
    def _local_confidence(thought: Any) -> float:
        if not isinstance(thought, Mapping):
            return 1.0
        try:
            return max(0.0, min(1.0, float(thought.get("confidence", 1.0))))
        except (TypeError, ValueError):
            return 1.0
    ...
    def _candidate_distribution(self, candidates: Sequence[Candidate], action_count: int) -> List[float]:
        raw_scores = [0.0 for _ in range(action_count)]
        for candidate in candidates:
            if 0 <= candidate.index < action_count:
                raw_scores[candidate.index] = candidate.base_score
        return softmax(raw_scores, temperature=22.0)
    ...
    def _parse_message_distribution(self, raw_message: str, vector: Sequence[float], context: GameContext) -> List[float]:
        if self.hp.strict_repro:
            return parse_message_distribution(raw_message)
        dist = self._contextual_prompt_distribution(vector, context)
        try:
    ...
    def _should_ask_local_model(
        ...
        if (
            ...
            return False
        if self.hp.local_model_budget > 0 and self.local_model_used >= self.hp.local_model_budget:
            return False
    ...
    def _offline_update(self, episode: EpisodeRecord, snapshot: BeliefSnapshot) -> None:
        ...
        if self.hp.evolve:
            ...
            if topology_step:
                ...
                if self.hp.strict_repro:
                    artifacts = getattr(self.evaluator, "last_artifacts", None)
                    prompt_gradients = getattr(artifacts, "prompt_gradients", {}) if artifacts is not None else {}
                    apply_bidirectional_gradient_refinement(
                        self.topology,
                        self.population,
                        gradients,
                        prompt_gradients,
                ...
                if self.hp.strict_repro:
                    project_inbound_simplex(self.topology)
        ...
        stats_payload = {
            ...
            "ablation": {
                ...
                "strict_repro": self.hp.strict_repro,
            },
            ...
            "node_local_model": {
                "attempts": self.node_model_used,
                "successes": self.node_model_successes,
                "failures": self.node_model_failures,
                "by_node": self.node_model_by_node,
            },
            "edge_local_model": {
...
def metrics_reference() -> Dict[str, Any]:
    return {
        ...
        "strict_algorithm": "Set COTE_STRICT_REPRO=1 for q_phi parsing, replay artifacts, strict EL-BDPEA sampling, and projected bidirectional Phase-D updates.",
