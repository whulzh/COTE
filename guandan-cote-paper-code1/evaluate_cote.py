...
def aggregate_model_control(stats_items: Iterable[Dict[str, Any]]) -> Dict[str, int]:
    ...
    for stats in stats_items:
        ...
        node = stats.get("node_local_model") if isinstance(stats, dict) else None
        ...
        if isinstance(node, dict):
            totals["node_attempts"] += int(node.get("attempts", 0) or 0)
            totals["node_successes"] += int(node.get("successes", 0) or 0)
            totals["node_failures"] += int(node.get("failures", 0) or 0)
